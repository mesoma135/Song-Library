...
//GET all with artist (for songs page)
exports.getAllWithArtist = async (req, res) => {
    try {
        const [songs] = await db.promise().query(`
            SELECT s.SongID, s.Title, s.NumPlays, s.Length, a.Name AS ArtistName
            FROM Song s
            JOIN Artist a ON s.ArtistID = a.ArtistID
            ORDER BY s.NumPlays DESC
        `);
        res.json(songs);
    } catch (err) {
        res.status(500).json({ error: "Failed to fetch songs with artist", details: err });
    }
};
